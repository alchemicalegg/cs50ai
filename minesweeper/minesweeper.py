...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        if cells:
            ...
            self.revelation(sentence)

    def revelation(self, new_sentence):
        if not new_sentence.cells:
            raise Exception("You can't have a revelation about no cells")
        if new_sentence in self.knowledge:
            return
        self.knowledge.append(new_sentence)
        done = False
        while not done:
            done = not self.find_mines()
            done = not self.find_safes() and done
        self.infer()
    ...
    def make_random_move(self):
        ...
        for row in range(0, self.height):
            for col in range(0, self.width):
                candidate = (row, col)
                if candidate not in self.safes and candidate not in self.mines and candidate not in self.moves_made:
